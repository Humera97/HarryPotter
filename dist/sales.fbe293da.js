...
var xPositionScale = d3.scaleLinear().range([0, width]).domain([1, 27]);
var yPositionScale = d3.scaleLinear().range([height, 0]).domain([0, 400000000]);
var colorScale = d3.scaleOrdinal().range(['#7fc97f', '#beaed4', '#fdc086', '#ffff99', '#386cb0', '#f0027f']); // Create a d3.line function that uses your scales

var area = d3.line().x(function (d) {
  return xPositionScale(d.Year);
}).y(function (d) {
  return yPositionScale(d.Value);
});
d3.csv(require('/data/boxoffice.csv')).then(ready).catch(function (err) {
  console.log(err);
});
...
function ready(datapoints) {
  ...
  svg.selectAll('path').data(nested).enter().append('path').attr('d', function (d) {
  ...
  svg.selectAll('text').data(nested).enter().append('text').attr('font-size', '14px').attr('font-family', "Averia Libre").text(function (d) {
    return d.key;
    ...
    return yPositionScale(d.values[d.values.length - 1].Value);
  }).attr('dx', 5).attr('dy', 5); // Add your axes

  var xAxis = d3.axisBottom(xPositionScale).tickFormat(d3.format('d'));
  svg.append('g').attr('class', 'axis x-axis').attr('transform', 'translate(0,' + height + ')').call(xAxis).attr('stroke-width', '4px').attr('font-family', 'Averia Libre').attr('font-size', '14').attr('color', '#B1BCCB');
  var yAxis = d3.axisLeft(yPositionScale);
  svg.append('g').attr('class', 'axis y-axis').call(yAxis).attr('stroke-width', '4px').attr('font-family', 'Averia Libre').attr('font-weight', '800').attr('font-size', '14').attr('color', '#CBD2E2');
}
},{"d3":"../node_modules/d3/index.js","/data/boxoffice.csv":"data/boxoffice.csv"}]},{},["scripts/sales.js"], null)
//# sourceMappingURL=/sales.fbe293da.js.map